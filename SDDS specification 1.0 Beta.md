...
也可以通过基本类型bytes读出字节，然后再通过format,formula,after_action等属性指定对应的的扩展函数实现。
...
before_action：json,用以指明，当前节点数据处理前要调用的用户自定义函数。调用发生的时间是：函数解码时，在初始化以后，读取数据之前，编码时，在初始化以后，写入数据之前。
    
after_action：json 用以指明，当前节点数据处理后要调用的用户自定义函数。调用发生的时间是：函数解码时，在初始化以后，读取数据之后，编码时，在初始化以后，写入数据之后。
...
| 属性名	| 数据类型 |  说明 |
|---------------|-----------|----------|   
| after_action | string | 定义读写操作后调用的自定义函数，逗号分隔 
| before_action | string | 定义读写操作前调用的自定义函数，逗号分隔
...
| value | mixed | 当前节点的值，可以是读取的，也可以是写入的。mixed为复合类型，即可能为不同的类型。
